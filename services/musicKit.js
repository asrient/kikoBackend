...
var spotify = common.spotify;

var app = null;


setUser = (usr, callback) => {
    ...
    getNewToken = (source, callback) => {
        ...
        spotify.setRefreshToken(source['refresh_token']);
...
getTrackLength = (ids, cb) => {
    console.log('///////getting track length////////////');
    getTrack(ids, (data, err) => {
        if (data == null) {
            cb(null, err)
        }
        else {
...
getTrack = (ids, cb) => {
    ...
    fromSpotifyIsrc = (isrc, cb) => {
            ...
            console.log('searching track on spotify..');
            spotify.searchTracks('isrc:' + isrc).then(
                (data) => {
                ...
                if (song != null&&song.spotifyId!=undefined) {
                  ...
                  console.warn('..song not on db..');
                        ...
                        if (ids.spotifyId != undefined) {
                            console.warn('getting track from spotify. [getTrack-musickit]',ids)
...
musickit.getSpotifyId('USUM71900409',(id,err)=>{
    if(id!=null){
  reply('SUCCESS',{time:common.time(),spotifyId:id});
    }
  else{
    reply('FAILED',err);
  }
